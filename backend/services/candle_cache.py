...
async def _fetch_range(session, symbol: str, start_ms: int, end_ms: int,
                       job: Dict = None, pace: float = 0.06) -> CandleArray:
    ...
    return CandleArray.from_matrix(np.concatenate(blocks))
...
async def _fetch_range_parallel(session, symbol: str, start_ms: int, end_ms: int,
    ...
    gedrosselt, dass die Binance-Ratelimits eingehalten werden."""
