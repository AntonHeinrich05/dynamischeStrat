...
const MODES = [
  ...
  { id: 'dynamic', title: 'Dynamische Strategie', desc: 'Erkennt Marktregime automatisch (ohne Lookahead) und optimiert pro Regime eine eigene Trade-Konfiguration – immer im Vergleich zur statischen Benchmark.' },
];
...
export default function Optimizer({ onClose }) {
  ...
  const run = async () => {
    ...
    if ((mode === 'discovery' || mode === 'combo') && indicators.length === 0) { toast.error('Mind. 1 Indikator anhaken'); return; }
    ...
    try {
      const res = await fetch(`${API_URL}/api/optimizer/run`, {
        ...
        body: JSON.stringify({
          ...
          base_strategy_id: (mode === 'discovery' || mode === 'combo') && baseStrategy ? baseStrategy : undefined,
          execution,
